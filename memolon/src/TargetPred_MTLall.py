...
##### creating TargetPred lexicons (MTL_all, so MTL across ALL 8 variables)
...
for iso in iso_codes:
    ...
    # Multi Task Learning across all 8 variables (VAD+JASFD)
    net = model.Net(8)
    ...
    target_pred['joy'] = predictions[:, 3]
    target_pred['anger'] = predictions[:, 4]
    target_pred['sadness'] = predictions[:, 5]
    target_pred['fear'] = predictions[:, 6]
    target_pred['disgust'] = predictions[:, 7]
    ...
    print(sorted.head())
    print(sorted.shape)
    sorted.to_csv(cs.TARGETPRED_MTL_all / '{}.tsv'.format(iso), sep='\t', index=True, encoding='utf-8')
